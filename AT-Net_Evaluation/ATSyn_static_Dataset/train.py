...
parser.add_argument('-exp_name', help='directory for saving the networks of the experiment', default='./results_ATSyn/',type=str)
...
if category == 'distort':
    num_epochs = 50
    train_data_dir = '/content/ATSyn_static_Dataset/'
...
#net.load_state_dict(torch.load('/content/Master/MyDrive/AT-Net-main/AT-Net-main/results/distort_epoch3'))
#net_var.load_state_dict(torch.load('/content/Master/MyDrive/AT-Net-main/AT-Net-main/results/distort_var_epoch3'))
...
# --- Load training data and validation/test data --- #
train_data_loader = DataLoader(TrainData(crop_size, train_data_dir), batch_size=train_batch_size, shuffle=True, num_workers=4,pin_memory=True)
...
#-------------------------------------------------------------------------------------------------------------
    ...
    for  train_data in tqdm(train_data_loader):
        ...
        Var1 = torch.zeros([shape[0],shape[1],shape[2],shape[3]], dtype=torch.float32,  device=device)
        Num_itr = 10
        x_output = torch.zeros([Num_itr,shape[0],shape[1],shape[2],shape[3]], dtype=torch.float32, device=device)
        ...
        loss = smooth_loss + ang_loss + 0.25*grad_loss
